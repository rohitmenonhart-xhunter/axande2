    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      <div className={`flex flex-col space-y-2 mb-3 transition-all duration-300 ${isOpen ? 'opacity-100 transform translate-y-0' : 'opacity-0 pointer-events-none transform translate-y-4'}`}>
        <button 
          ...
          onClick={() => window.open('https://calendly.com/example/meeting', '_blank')}
        >
          <span className="mr-2">📅</span> Prendre RDV
        </button>
        
        <button 
          className="bg-white shadow-lg hover:shadow-xl text-gray-800 font-medium rounded-full py-3 px-5 transition-all duration-300 flex items-center"
          onClick={() => window.open('mailto:contact@axande.com', '_blank')}
        >
          <span className="mr-2">✉️</span> Nous contacter
        </button>
        
        <button 
          className="bg-blue-600 shadow-lg hover:shadow-xl text-white font-medium rounded-full py-3 px-5 transition-all duration-300 flex items-center"
